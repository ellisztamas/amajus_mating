...
folder = "005.results/004_mcmc_restrict_kurtosis/output/"
...
# Summarise mating events and dispersal
{
    ...
    'dispersal>100m'    : np.sum(data.mating['obs']['distance'] > 100),
    'dispersal>500m'    : np.sum(data.mating['obs']['distance'] > 500),
    'dispersal>1000m'   : np.sum(data.mating['obs']['distance'] > 1000),
}
...
boundaries  = [-827, -70, 70, 244, 270, 560]
...
for r in range(1, len(boundaries)):
    w_obs = (obs.loc[region_obs == r]['simple_colour_father'] == obs.loc[region_obs == r]['simple_colour_mother']).mean()
    w_exp = (exp.loc[region_exp == r]['simple_colour_father'] == exp.loc[region_exp == r]['simple_colour_mother']).mean()
    print(r, w_obs/w_exp)

# Turn the following loop into a functions that takes genotype and boundaries as a function
# apply that to phenotypes, ros, sulf
# Repeat for assortment, or don't bother

rel_fitness = []
for geno in ["FR", "hybrid", "Ye"]:
    # Absolute fitness each genotype
    abs_obs = [(obs['simple_colour_father'] == geno).sum() for geno in ["FR", "hybrid", "Ye"]]
    abs_exp = [(exp['simple_colour_father'] == geno).sum() for geno in ["FR", "hybrid", "Ye"]]
    # Relative fitness of each genotype
    rel_obs = abs_obs / np.max(abs_obs)
    rel_exp = abs_exp / np.max(abs_exp)

    rel_fitness = rel_fitness + [['obs', 'all', np.nan, np.nan] + list(rel_obs)]
    rel_fitness = rel_fitness + [['exp', 'all', np.nan, np.nan] + list(rel_exp)]

    for r in range(1, len(boundaries)):      
        # Absolute fitness each genotype
        abs_obs = [(obs.loc[region_obs == r]['simple_colour_father'] == geno).sum() for geno in ["FR", "hybrid", "Ye"]]
        abs_exp = [(exp.loc[region_exp == r]['simple_colour_father'] == geno).sum() for geno in ["FR", "hybrid", "Ye"]]
        # Relative fitness of each genotype
        rel_obs = abs_obs / np.max(abs_obs)
        rel_exp = abs_exp / np.max(abs_exp)

        rel_fitness = rel_fitness + [['obs', r, boundaries[r-1], boundaries[r]] + list(rel_obs)]
        rel_fitness = rel_fitness + [['exp', r, boundaries[r-1], boundaries[r]] + list(rel_exp)]
pd.DataFrame(rel_fitness)
