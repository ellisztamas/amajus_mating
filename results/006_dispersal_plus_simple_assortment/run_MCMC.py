...
from mcmc import *
from mating import assortment_probs

def run_MCMC(paternity_arrays, distance_matrix, maternal_phenotypes, paternal_phenotypes, initial_parameters, proposal_sigma, nreps, output_dir, chain_name, thin=1, max_distance = np.inf):
    """
    ...
    maternal_phenotypes: series
        Vector of maternal phenotypes for assortative mating.
    paternal_phenotypes: series
        Vector of paternal phenotypes for assortative mating.
    ...
    for i in tqdm(range(nreps)):
        ...
        # Multiply by probability of mating given assortment probabilities
        prob_drawn += assortment_probs(
            focal = maternal_phenotypes,
            candidates= paternal_phenotypes,
            params={
                'FR':     new_model['assortment'],
                "hybrid": new_model['assortment'],
                "Ye":     new_model['assortment']
            }
        )
        ...
        prior_probs = {
            ...
            'shape'     : gma.pdf(new_model['shape'],   a=20, scale = 1/5),
            'scale'     : gma.pdf(new_model['shape'],   a=6, scale = 50),
            'assortment': beta.pdf(new_model['missing'],a=1.1,   b=1.1)
        }
