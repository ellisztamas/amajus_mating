...
from amajus_mating import dispersal
from amajus_mating import mcmc
...
class faps_data(object):
    ...
    def run_MCMC(self, initial_parameters, proposal_sigma, priors, nreps, output_dir, chain_name, thin=1, max_distance = np.inf):
        ...
        current_model['log_prior']     = 10e-12
        current_model['loglik']        = 10e-12
        current_model['log_posterior'] = 10e-12
        ...
        # UPDATE PARAMETERS
            ...
            current_model['log_prior'] = np.log(prior_probs).sum()
            ...
            if(i in np.arange(start = 0, stop = nreps, step=thin)):
                mcmc.write_output(out_file, i, current_model, decimals=3, time0=t0)
